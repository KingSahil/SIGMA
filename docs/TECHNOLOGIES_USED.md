...
| **Math & Vectors**      | **NumPy**                           | >= 1.22.x                                        | Vectorized sample analysis, complex64 arrays, RMS amplitude, peak detection, FFT-based symbol rate and demodulation.                                                              |
| **Scientific I/O**      | **SciPy** (`scipy.io.wavfile`)      | >= 1.9.x                                         | Reading, validating, and writing 16-bit PCM RIFF WAV audio files.                                                                                                                 |
| **DSP Algorithms**      | **In-house NumPy DSP**              | `src/sigma_symbol_rate.py`, `src/sigma_sample_rate.py`, `src/sigma_demod.py` | Cyclostationary symbol-rate estimation (Welch + parabolic interpolation), ranked sample-rate provenance resolution, RRC matched filtering, 4th-power carrier recovery, timing-phase search, constellation decision slicing. |
...
- **Why it is used**:
  ...
  - Powers the symbol-rate estimator (`|x[n]|²` envelope, Welch-averaged FFT, parabolic sub-bin interpolation) and the demodulator (RRC filter synthesis, `np.convolve` matched filtering, `np.angle` carrier recovery).

> [!WARNING]>   
> **Radioconda ships numpy 2.2.x, which differs from a typical dev install in two>   
> ways that have already caused a crash:**
>
> - `np.fft.rfft` **rejects complex input outright** — `TypeError: ufunc
>   'rfft_n_even' not supported for the input types` — even for>     
>   `np.ones(1024, dtype=np.complex128)`. Real input is fine. Use `np.fft.fft`>     
>   and slice the first half instead.
> - `complex128 ** 2` can silently drop the complex dtype. Prefer repeated>     
>   multiplication.
>
> Code that runs on a dev numpy (2.5.x) can therefore crash on a judge's machine.>   
> Verify with `& "$env:USERPROFILE\radioconda\python.exe"`.
